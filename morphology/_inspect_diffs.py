...
print('=== Vraies divergences (squelette consonnes différent) ===\n')
n = 0
for r in data:
    a = r['almaany']
    if a.get('status') != 'ok':
    ...
    for field, ours, theirs in [
        ...
        if not ours or not theirs or ours == 'NULL':
            continue
        if clean(ours) == clean(theirs):
            continue
        msg = "{cat:<18} {lem:<14} {f}: nous=`{o}`  vs  Almaany=`{t}`".format(
            cat=r['category'], lem=r['lemma_ar'], f=field, o=ours, t=theirs
        )
        print(msg)
        n += 1
        if n >= 60:
