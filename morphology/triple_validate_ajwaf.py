"""Triple cross-validation pour أجوف : nous / reverso / cooljugator / studyquran.

Stratégie de confiance :
- HC : ≥ 2 sources externes d'accord et différentes de nous
- MC : 1 source diffère, autres absentes ou d'accord avec nous
- HOMOGRAPH : reverso bascule sur autre racine (past skel diff)
- AGREE : tout match

Sortie : ajwaf_triple_report.md + ajwaf_triple_overrides.json (HC proposals)
"""
...
# Per-verb analysis
hc_proposals = {}      # high-confidence overrides
agreements = 0
verbs_with_hc = 0
homographs = []
diff_details = []
...
def filter_safe(field, value):
    ...
    v_devoc = devoc(value)
    if v_devoc in {'الاجوف','السالم','المهموز','الفعل','الماضي','المضارع'}: return False
...
for can in ajwaf:
    ...
    # Detect homograph (reverso shifts to different root)
    ...
    if rev_match:
        ...
        if rev_past_sk and rev_past_sk != past_sk:
            ...
            homographs.append({'root': root, 'past': can['past_3ms'], 'src': 'reverso'})
...
# Stats
print(f'Total أجوف Form I active : {len(ajwaf)}')
print(f'Sans aucune source : {len(no_sources)}')
print(f'Homographes reverso : {len(homographs)}')
print(f'HC overrides proposés : {sum(len(v) for v in hc_proposals.values())} champs sur {len(hc_proposals)} verbes')

# Report
lines = ['# Triple cross-validation أجوف (reverso + cooljugator + studyquran)', '']
lines.append(f'- Total verbes أجوف Form I active : {len(ajwaf)}')
lines.append(f'- HC propositions : {len(hc_proposals)} verbes, {sum(len(v) for v in hc_proposals.values())} champs')
lines.append(f'- Homographes reverso détectés : {len(homographs)}')
lines.append('')
...
with open(os.path.join(DIR, 'ajwaf_triple_report.md'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

for key, fields_dict in hc_proposals.items():
    fields_dict['comment'] = f'Triple-validated (≥2 sources d\'accord)'

with open(os.path.join(DIR, 'ajwaf_triple_overrides.json'), 'w', encoding='utf-8') as f:
    json.dump(hc_proposals, f, ensure_ascii=False, indent=2)

print(f'\n→ ajwaf_triple_report.md')
print(f'→ ajwaf_triple_overrides.json')
