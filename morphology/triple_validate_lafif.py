"""Triple cross-validation pour لفيف (deux lettres faibles, و/ي dans 2 positions)."""
...
# Load lafif : ≥ 2 lettres faibles (و/ي) parmi R1/R2/R3
out = subprocess.run(
    ['C:/MAMP/bin/mysql/bin/mysql.exe', '-u', 'root', '-proot',
     ...
     "AND ("
     "  (SUBSTRING_INDEX(root_ar,' ',1) IN ('و','ي') AND SUBSTRING_INDEX(root_ar,' ',-1) IN ('و','ي'))"
     "  OR (SUBSTRING_INDEX(SUBSTRING_INDEX(root_ar,' ',2),' ',-1) IN ('و','ي') AND SUBSTRING_INDEX(root_ar,' ',-1) IN ('و','ي'))"
     "  OR (SUBSTRING_INDEX(root_ar,' ',1) IN ('و','ي') AND SUBSTRING_INDEX(SUBSTRING_INDEX(root_ar,' ',2),' ',-1) IN ('و','ي'))"
     ")"],
...
def filter_safe(field, value):
    ...
    v_devoc = devoc(value)
    if v_devoc in {'الاجوف','السالم','المهموز','الفعل','الماضي','المضارع','الناقص','المعتل','اللفيف'}: return False
...
print(f'Total لفيف Form I active : {len(lafif)}')
...
lines = ['# Triple cross-validation لفيف', '']
lines.append(f'- Total verbes لفيف Form I active : {len(lafif)}')
...
with open(os.path.join(DIR, 'lafif_triple_report.md'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

for key, fields_dict in hc_proposals.items():
    fields_dict['comment'] = 'Triple-validated لفيف (≥2 sources)'
with open(os.path.join(DIR, 'lafif_triple_overrides.json'), 'w', encoding='utf-8') as f:
    json.dump(hc_proposals, f, ensure_ascii=False, indent=2)

print(f'\n→ lafif_triple_report.md')
print(f'→ lafif_triple_overrides.json')
