"""Triple cross-validation pour مثال (assimilé : R1 faible و/ي, R2 et R3 non-faibles)."""
...
# Load mithal : R1 = و ou ي, mais R2 et R3 non-faibles
out = subprocess.run(
...
def filter_safe(field, value):
    ...
    v_devoc = devoc(value)
    if v_devoc in {'الاجوف','السالم','المهموز','الفعل','الماضي','المضارع','الناقص','المعتل','المثال'}: return False
...
print(f'Total مثال Form I active : {len(mithal)}')
...
lines = ['# Triple cross-validation مثال', '']
lines.append(f'- Total verbes مثال Form I active : {len(mithal)}')
...
with open(os.path.join(DIR, 'mithal_triple_report.md'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

for key, fields_dict in hc_proposals.items():
    fields_dict['comment'] = 'Triple-validated مثال (≥2 sources)'
with open(os.path.join(DIR, 'mithal_triple_overrides.json'), 'w', encoding='utf-8') as f:
    json.dump(hc_proposals, f, ensure_ascii=False, indent=2)

print(f'\n→ mithal_triple_report.md')
print(f'→ mithal_triple_overrides.json')
