"""Triple cross-validation pour مضعف (assimilé : R1 faible و/ي, R2 et R3 non-faibles)."""
...
# Load mudaaf : R2 == R3 (et non-faibles)
out = subprocess.run(
...
def filter_safe(field, value):
    ...
    v_devoc = devoc(value)
    if v_devoc in {'الاجوف','المضعف','المهموز','الفعل','الماضي','المضارع','الناقص','المعتل','المضعف'}: return False
...
print(f'Total مضعف Form I active : {len(mudaaf)}')
...
lines = ['# Triple cross-validation مضعف', '']
lines.append(f'- Total verbes مضعف Form I active : {len(mudaaf)}')
...
with open(os.path.join(DIR, 'mudaaf_triple_report.md'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

for key, fields_dict in hc_proposals.items():
    fields_dict['comment'] = 'Triple-validated مضعف (≥2 sources)'
with open(os.path.join(DIR, 'mudaaf_triple_overrides.json'), 'w', encoding='utf-8') as f:
    json.dump(hc_proposals, f, ensure_ascii=False, indent=2)

print(f'\n→ mudaaf_triple_report.md')
print(f'→ mudaaf_triple_overrides.json')
