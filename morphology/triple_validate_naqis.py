"""Triple cross-validation pour ناقص (défectueux R3 = و/ي).
Adapté de triple_validate_ajwaf.py.
"""
...
# Load naqis : R3 = و ou ي, mais R2 doit être NON-faible (sinon c'est lafif/ajwaf)
out = subprocess.run(
    ['C:/MAMP/bin/mysql/bin/mysql.exe', '-u', 'root', '-proot',
     ...
     "AND SUBSTRING_INDEX(root_ar,' ',-1) IN ('و','ي') "
...
def filter_safe(field, value):
    ...
    v_devoc = devoc(value)
    if v_devoc in {'الاجوف','السالم','المهموز','الفعل','الماضي','المضارع','الناقص','المعتل'}: return False
...
print(f'Total ناقص Form I active : {len(naqis)}')
...
lines = ['# Triple cross-validation ناقص', '']
lines.append(f'- Total verbes ناقص Form I active : {len(naqis)}')
...
with open(os.path.join(DIR, 'naqis_triple_report.md'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

for key, fields_dict in hc_proposals.items():
    fields_dict['comment'] = 'Triple-validated ناقص (≥2 sources)'
with open(os.path.join(DIR, 'naqis_triple_overrides.json'), 'w', encoding='utf-8') as f:
    json.dump(hc_proposals, f, ensure_ascii=False, indent=2)

print(f'\n→ naqis_triple_report.md')
print(f'→ naqis_triple_overrides.json')
