"""Triple cross-validation pour سالم (assimilé : R1 faible و/ي, R2 et R3 non-faibles)."""
...
# Load salim : aucune lettre faible (و, ي, ا), pas géminé, racine = 3 lettres
out = subprocess.run(
    ['C:/MAMP/bin/mysql/bin/mysql.exe', '-u', 'root', '-proot',
     ...
     "AND SUBSTRING_INDEX(root_ar,' ',1) NOT IN ('و','ي','ا') "
     "AND SUBSTRING_INDEX(SUBSTRING_INDEX(root_ar,' ',2),' ',-1) NOT IN ('و','ي','ا') "
     "AND SUBSTRING_INDEX(root_ar,' ',-1) NOT IN ('و','ي','ا') "
     "AND SUBSTRING_INDEX(SUBSTRING_INDEX(root_ar,' ',2),' ',-1) != SUBSTRING_INDEX(root_ar,' ',-1)"],
...
def filter_safe(field, value):
    ...
    v_devoc = devoc(value)
    if v_devoc in {'الاجوف','السالم','المهموز','الفعل','الماضي','المضارع','الناقص','المعتل','السالم'}: return False
...
print(f'Total سالم Form I active : {len(salim)}')
...
lines = ['# Triple cross-validation سالم', '']
lines.append(f'- Total verbes سالم Form I active : {len(salim)}')
...
with open(os.path.join(DIR, 'salim_triple_report.md'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

for key, fields_dict in hc_proposals.items():
    fields_dict['comment'] = 'Triple-validated سالم (≥2 sources)'
with open(os.path.join(DIR, 'salim_triple_overrides.json'), 'w', encoding='utf-8') as f:
    json.dump(hc_proposals, f, ensure_ascii=False, indent=2)

print(f'\n→ salim_triple_report.md')
print(f'→ salim_triple_overrides.json')
